...
const Darkness = styled.div`
  ...
  animation: ${(props) => (props.secondCheck ? "darkness 0.4s ease-in-out" : "light 0.4s ease-in-out")};
  @keyframes darkness {
    0%{
      background: #0002;
    }
    ...
    <Container isChild={isChild}>
        ...
        {editOpen ? (
            ...
            <Wrapper >
                <Details onMouseEnter={() => setThreeDots(true)} onMouseLeave={() => setThreeDots(false)}>
                    ...
                    <Right>
                        <ThreeDotsContainer onClickCapture={handleToggleOptions}
                            ...
                            glow={glow}
                        >
                            <TeenyiconsMoreVerticalOutline  style={{display: threeDots ? "grid" : "none" }} />
                        </ThreeDotsContainer>
                        {openOptions && (
                            <Options 
                            optionRef= {optionRef} 
                ...
                {openReplies &&(
                    replies.map((reply)=>(
                        <Comment key={reply._id} comment={reply} isChild={true} currentUser={currentUser} />
                    ))
                )}
            </Wrapper>
        )}
        {darkEffect && (
            <Darkness secondCheck={secondCheck} />
        )}
    </Container>
  )
}
