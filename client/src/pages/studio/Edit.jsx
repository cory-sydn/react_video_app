...
import { Wrapper, UploaderBox, UploaderHeader, UploaderFooter, HeaderTitle, CloseBtn, HiddenInput} from '../../components/upload/Upload';
import { Divided, Title3, Title4, Left, Right, DetailsForm, Input, DescInput, ThumbnailImg, ThumbnailInputLabel} from '../../components/upload/Upload';
import { Card, Screen, Item, ItemTitle, ItemExplain, VideoLink, ProcessDisplay, VideoFrame, ConfirmButton} from '../../components/upload/Upload';
...
const Edit = ({ openEdit, setOpenEdit }) => {
  ...
  const handleInitialState = useCallback(() => {
    ...
    setVideoDetails((videoDetails) => ( videoDetails = {
  ...
  const handleConfirm = async(e) => {
    e.preventDefault()
    try {
      const res = await axios.put(`http://localhost:8800/api/videos/${video._id}`, {...videoDetails}, {withCredentials:true})
      ...
      if (res.status === 200) {
        if(video.imgUrl !== videoDetails.imgUrl) {
          ...
          await deleteObject(imageRef).then(() => {
            console.log("Previous version of video poster successfully deleted.");
          }).catch((err) =>{
            console.log(err);
          })
        }
  ...
  const handleCancel = useCallback(() => {
    if (!secondCheck){
      setWarning({state: true, message: "Close without saving?\n\nAll changes will be lost!"})
      setSecondCheck(true)
    } else {
        ...
        <UploaderBox>
            ...
            <Left>
              <DetailsForm
                ...
                <ThumbnailInputLabel
                  ...
                  htmlFor="thumbnail"
                  >
                  {videoDetails?.imgUrl && <ThumbnailImg src={videoDetails?.imgUrl} /> }
                  {percentage > 0 && percentage < 100 ? ( 
                      <ProcessDisplay percentage={Math.round(percentage)}></ProcessDisplay> 
                    ) : percentage === 100 && (<ThumbnailImg src={videoDetails?.imgUrl}></ThumbnailImg>)
                  }
                </ThumbnailInputLabel>                        
                <HiddenInput
                  id="thumbnail"
                ...
                />
                <Title4>Tags</Title4>
                <ItemExplain>Tags can be useful and make your video easy to find, especially if content in your video is commonly misspelled.<br/><br/>Please separate the tags with commas.</ItemExplain>
                <Input
          ...
          <UploaderFooter>
            ...
            <ConfirmButton 
              ...
              disabled={uploadTask._state === "running" ? true : suspendConfirm}
              style={{cursor: uploadTask._state === "running" ? "progress" : suspendConfirm ?  "not-allowed" : "pointer"}}
