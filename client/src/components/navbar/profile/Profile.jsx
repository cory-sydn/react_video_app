...
import GlowingButton, {	BtnContainer,
...
const BtnContainerProfile = styled(BtnContainer)`
	background: ${({ theme }) => theme.bgLighter};
`;
...
const Profile = () => {
	...
	return (
		<Container>
			{currentUser === null ? (
				<Section>
					<BtnContainerProfile onClick={() => setDropdown(true)}>
						<GlowingButton 
				...
				<Section>
					<UploadSectionButton onClick={activateUploading}>
						<GlowingButton
							icon={openUpload ? <GlowingVideoIcon /> : <CarbonVideoAdd />}
						/>
					</UploadSectionButton>
					{openUpload && (
						<Upload
					...
					<NotificationButton>
						<GlowingButton icon={<NotificationsNoneIcon />} />
					</NotificationButton>
					{currentUser && (
			...
			{dropdown && <DropDown setDropdown={setDropdown} menuRef={menuRef} />}
		</Container>
	);
};
